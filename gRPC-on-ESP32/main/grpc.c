...
bool grpc_init(grpc_init_t config)
{
    ...
    if (ret != pdPASS)
    {
        ESP_LOGE(TAG, "Failed to create http2 task, ret: %d", ret);
        return false;
    }
    ...
    if (ret != pdPASS)
    {
        ESP_LOGE(TAG, "Failed to create GRPC task, ret: %d", ret);
        return false;
    }
    ESP_LOGI(TAG, "%d", ret);
...
bool grpc_call_proc(char *path, char *proc, uint8_t *data, uint32_t len)
{
    ...
    if (len >= max_len)
    {
        ESP_LOGE(TAG, "Specified Length of %d exceeds the maximum allowed data size of %d", len, max_len);
        return false;
    }
    ...
    if (ret < 0)
    {
        ESP_LOGE(TAG, "Submit Request failed: %d", ret);
...
static bool grpc_task_start_connection()
{
    ...
    if (bools.connected)
    {
        ESP_LOGW(TAG, "you shouldn't be here");
        return false;
    }
...
static void grpc_task()
{
    ESP_LOGI(TAG, "Entered GRPC task");
...
static bool http2_execute()
{
    ...
    if (ret != 0)
    {
        ESP_LOGE(TAG, "[sh2-execute] HTTP2 session send failed, ret: %d (%s)", ret, nghttp2_strerror(ret));
        return false;
    }
    ...
    if (ret != 0)
    {
        ESP_LOGE(TAG, "[sh2-execute] HTTP2 session recv failed, ret: %d (%s)", ret, nghttp2_strerror(ret));
        return false;
    }
...
static bool _connect()
{
    ...
    // we will include the default cert bundle included by esp-tls
    ...
    ESP_LOGI(TAG, "Setting up HTTP2 connection with uri: %s", conn_data.uri);
    ...
    if (ret != 0)
    {
        ESP_LOGE(TAG, "New http2 session failed, ret: %d (%s)", ret, nghttp2_strerror(ret));
    ...
    for (;;)
    {
        ...
        // if(!wifi_get_internet_status())
        // {
        //     ESP_LOGW("Lost internet connection");
        //     goto error;
        // }
...
static int send_post_data(char *buf, size_t length, uint32_t *data_flags)
{
    ...
    if (copylen < length)
    {
        ESP_LOGI(TAG, "[SEND] Sending %d bytes", copylen);
...
static int callback_on_frame_send(nghttp2_session *session, const nghttp2_frame *frame, void *user_data)
{
    ESP_LOGD(TAG, "[frame-send] frame type %s", frame_type_to_str(frame->hd.type));
...
static int callback_on_frame_recv(nghttp2_session *session, const nghttp2_frame *frame, void *user_data)
{
    ...
    ESP_LOGD(TAG, "[frame-recv][sid: %d] frame type: %d (%s)", frame->hd.stream_id, frame->hd.type, frame_type_to_str(frame->hd.type));
    ...
    case NGHTTP2_WINDOW_UPDATE:
    {
        ...
        ESP_LOGI(TAG, "[Window Size] %d", w->window_size_increment);
    }
    break;
...
static int callback_on_stream_close(nghttp2_session *session, int32_t stream_id, uint32_t error_code, void *user_data)
{
    ...
    ESP_LOGI(TAG, "[RECV] Stream Closed");
    return 0;
}

static int callback_on_data_chunk_recv(nghttp2_session *session, uint8_t flags, int32_t stream_id, const uint8_t *data, size_t len, void *user_data)
{
    ESP_LOGD(TAG, "[data-chunk-recv][sid:%d] %lu bytes", stream_id, (unsigned long int)len);
    ...
    if (len)
    {
        ...
        ESP_LOGI(TAG, "Received data chunk with following lenght: %d", buffer_recv.len);
        ESP_LOGI(TAG, "-----------------------------------------");

        for (size_t i = 0; i < buffer_recv.len; ++i)
        {
            printf("%02X ", buffer_recv.buf[i]);
        }
        printf("\n");
        ESP_LOGI(TAG, "-----------------------------------------");
    }
...
static int callback_on_header(nghttp2_session *session, const nghttp2_frame *frame, const uint8_t *name, size_t namelen, const uint8_t *value, size_t valuelen, uint8_t flags, void *user_data)
{
    ...
    if (strcmp((char *)name, "grpc-status") == 0)
    {
        int sc = atoi((const char *)value);
        ESP_LOGI(TAG, "[hdr-recv] GRPC Status: %s (%d)", grpc_status_code_to_str((GRPCStatusCode)sc), sc);
    }
...
uint8_t *grpc_get_buffer_data()
{
    return buffer_recv.buf;
}

size_t grpc_get_buffer_length()
{
    return buffer_recv.len;
}
